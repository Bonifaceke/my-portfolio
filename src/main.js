...
import renderTerminalUI from './terminal.js';
...
document.querySelector('#app').innerHTML = `
  ...
  <section class="relative min-h-screen flex flex-col justify-center items-center text-center px-6 bg-gradient-to-br from-white to-gray-100 text-gray-900 overflow-hidden fade-in">
    ...
    <div class="z-10 bg-white/30 backdrop-blur-md rounded-xl p-10 shadow-xl border border-white/20 max-w-3xl w-full flex flex-col items-center">
      ...
      <!-- Name -->
 <h1 class="glitch text-5xl sm:text-6xl md:text-7xl font-extrabold leading-tight mb-4 text-center relative" data-text="Boniface Mugo">
  Boniface Mugo
</h1>
<!-- Tagline -->
<p id="tagline" class="text-lg sm:text-xl md:text-2xl text-gray-700 max-w-2xl mb-4"></p>
...
// Append About & Skills sections

document.querySelector('#app').appendChild(renderAbout());
document.querySelector('#app').appendChild(renderSkills());  
document.querySelector('#app').appendChild(renderProjects());
document.querySelector('#app').appendChild(renderExperience());
document.querySelector('#app').appendChild(renderCertifications());
document.querySelector('#app').appendChild(renderEducation());
document.querySelector('#app').appendChild(renderTerminalUI());
document.querySelector('#app').appendChild(renderContact());
document.querySelector('#app').appendChild(renderTestimonials());
document.querySelector('#app').appendChild(renderFooter());
...
window.addEventListener('load', () => {
  const text = "Cybersecurity Specialist · Ethical Hacker · Web & Software Developer · Visionary Leader";
  ...
  function typeTagline() {
    if (index < text.length) {
      ...
      index++;
      setTimeout(typeTagline, 40); // Speed of typing
    }
  }
